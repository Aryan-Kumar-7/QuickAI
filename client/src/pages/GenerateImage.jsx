      ...
      <form onSubmit={onSubmitHandler} action="" className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='my-6 flex items-center gap-2'>
          <label className='relative cursor-pointer'>
            ...
            <span className='absolute left-1 top-1 bg-white w-3 h-3 rounded-full peer-checked:translate-x-4 transition'></span>
          </label>
          <p className='text-sm'>Make this image Public</p>
        </div>

        <br />
        <button disabled={loading} className='w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white px-4 py-2 mt-6 rounded-lg cursor-pointer text-sm'>
          {
          ...
          !content ? (
            <div className='flex-1 flex items-center justify-center'>
              <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
                ...
                <p>Enter a topic and click "Genrated image" to get started</p>
              </div>
            </div>
          ) : (
